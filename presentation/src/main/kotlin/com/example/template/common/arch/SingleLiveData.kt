...
import com.example.core.logger.lazyLogger
...
open class SingleLiveData<T> : MutableLiveData<T>() {
    ...
    private val log by lazyLogger(TAG)

    @MainThread
    override fun observe(owner: LifecycleOwner, observer: Observer<in T>) {
        if (hasActiveObservers()) {
            log.error("Multiple observers registered but only one will be notified of changes.")
        }
